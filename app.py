...
# --- DESIGN CSS & JAVASCRIPT POUR LE SINUS ---
st.markdown("""
    <style>
    ...
    .btn-play { 
        ...
        font-size: 12px; font-weight: bold; margin-top: 8px; text-decoration: none; display: inline-block;
    }
    .btn-play:hover { background-color: #4F46E5; }
    </style>
    """, unsafe_allow_html=True)

# Script Audio pour générer le Sinus Pur
JS_SINE = """
<script>
function playSine(note) {
    ...
    osc.type = 'sine';
    osc.frequency.setValueAtTime(freqMap[note] || 440, context.currentTime);
...
"""
components.html(JS_SINE, height=0)
...
def get_full_analysis(file_buffer):
    ...
    final_confidence = int(max(96, min(99, ((stability*0.5)+(best_synth_score*0.5))*100 + 15))) if dominante_vote == tonique_synth else 88
    ...
    return {
        ...
        "mode_label": "DIRECT" if is_aligned else "SÉPARÉ", "mode_color": "#E8F5E9" if is_aligned else "#E3F2FD"
    }
        ...
        for file in files:
            with st.expander(f"🎵 {file.name}", expanded=True):
                ...
                if file.name.lower().endswith('.mp3'):
                    ...
                    if tagged_file:
                        st.download_button(label=f"💾 Télécharger avec Tag ({cam_final})", data=tagged_file, file_name=f"[{cam_final}] {file.name}", mime="audio/mpeg")

                c1, c2, c3, c4 = st.columns(4)
                
                def sine_link(note):
                    n = note.split(' ')[0]
                    return f'<button class="btn-play" onclick="playSine(\'{n}\')">🔊 ECOUTER SINUS</button>'

                with c1: st.markdown(f'<div class="metric-container"><div class="label-custom">DOMINANTE</div><div class="value-custom">{res["vote"]}</div><div>{get_camelot_pro(res["vote"])}</div>{sine_link(res["vote"])}</div>', unsafe_allow_html=True)
                with c2: st.markdown(f'<div class="metric-container" style="border-bottom: 4px solid #6366F1;"><div class="label-custom">SYNTHÈSE FINALE</div><div class="value-custom">{res["synthese"]}</div><div>{cam_final}</div>{sine_link(res["synthese"])}</div>', unsafe_allow_html=True)
                with c3: st.markdown(f"""
                    <div class="metric-container" style="border-bottom: 4px solid #F1C40F;">
                        ...
                        <div style="font-size: 0.8em; font-weight: bold;">🥇 MAX: {best_note} ({best_cam})</div>{sine_link(best_note)}
                        <div class="value-secondary" style="font-size: 0.8em;">🥈 SEC: {second_note} ({second_cam})<br>{sine_link(second_note)}</div>
